...
class TakState(State):
    ...
    def bfs(self, S, T, player):
        ...
        while len(Q) > 0:
            ...
            for d in DIR:
                rr = r + d[0]
                cc = c + d[1]
                if 0 <= rr and rr < self.size and 0 <= cc and cc < self.size and parent[rr][
                    cc] == None and self.is_controlled_by(rr, cc, player):
                    Q.add((rr, cc))
                    parent[rr][cc] = (r, c)
                ...
                if len(self.board[row][col]) > 0 and self.get_top_piece(row, col)[1] == self.cur_player:
                    for d in DIR:
                        ...
                        for k in range(1, max_pieces + 1):
                            if self.can_move_top_k(row, col, row_next, col_next, k):
                                self.build_moves(row_next, col_next, k, [k], (row_next - row, col_next - col), row, col,
                                                 move_actions)
        return move_actions

    def build_moves(self, row_cur, col_cur, k, move, d, row_init, col_init, move_actions):
        move_actions.append(('move', row_init, col_init, d, move.copy()))
        row_next = row_cur + d[0]
        col_next = col_cur + d[1]
        for x in range(1, k):
            if self.can_move_top_k(row_cur, col_cur, row_next, col_next, x):
                move_next = move.copy()
                move_next.append(x)
                self.build_moves(row_next, col_next, x, move_next, d, row_init, col_init, move_actions)
            ...
            if 0 <= r and r < self.size and 0 <= c and c < self.size:
              ...
              for k in range(1, max_pieces + 1):
                if self.can_move_top_k(row, col, r, c, k):
                  print('building move from ({0}, {1}) in direction ({2}, {3}) moving {4} pieces'.format(row, col, d[0], d[1], k))
                  delta = (r - row, c - col)
                  move = ('move', row, col, delta, [k])
                  move_actions.append(move)
                  state_tmp = self.copy()
                  state_tmp.apply_action(move, False)
  ...
  """
    ...
    def apply_action(self, action, go_next=True):
...
def read_state_from_file(fn):
    ...
    for line in lines:
        if len(line) > 0:
            tmp.append(line)
    lines = tmp
    cur_player = int(lines[0])
    turn = int(lines[1])
    size = int(lines[2])
    max_height = int(lines[3])
    state = TakState(size, max_height)
    state.stones[0] = int(lines[4])
    state.stones[1] = int(lines[5])
    state.capstones[0] = int(lines[6])
    state.capstones[1] = int(lines[7])
    i = 8
    while i < len(lines):
        data = lines[i].split(' ')
        r = int(data[0])
