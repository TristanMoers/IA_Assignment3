        ...
        for event in events:
            ...
            if event.type == pygame.QUIT:
                ...
                if gui.grid.in_bounds(mouse_x, mouse_y):
                    ...
                    # check whether cell at row r and column c is empty
                    # and the current player still has pieces to play
                    if not self.state.is_empty(r, c) and self.state.get_top_piece(r, c)[
                        1] == self.state.cur_player and self.state.turn >= 3:
                        self.piece_selector = PieceSelector(self.state, r, c,
                                                            min(len(self.state.board[r][c]), self.state.size))
                        self.selected_cell = (r, c)
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 3:  # handle mouse right click
                ...
                if gui.grid.in_bounds(mouse_x, mouse_y):
                    ...
                    if gui.grid.are_adjacent(r, c, self.piece_selector.row, self.piece_selector.col) and can_move:
                        ...
                        self.selected_cell = (r, c)
                        self.state.move_top_k(self.piece_selector.row, self.piece_selector.col, r, c,
                                              self.piece_selector.nb_selected)
                        self.action = ('move', self.piece_selector.row, self.piece_selector.col, self.delta,
                                       [self.piece_selector.nb_selected])
                        ...
                        if self.piece_selector.max == 0 or not self.state.can_move_top_k(r, c, r + self.delta[0],
                                                                                         c + self.delta[1], 1):
                            self.piece_selector = None
                            self.function = self.wait_finish
                        else:
...
class TakGUI(GUI):
    ...
    def __init__(self, state, player_type, agents, timeout, wait):
        ...
        self.buttons = {}
        self.buttons['finish'] = Button(self, 'button_finish_active', 'button_finish_inactive',
                                        self.finish_human_action, False)
    ...
    def get_window_size(self):
        ...
        width = board_width + self.state.size * (
                    SPACINGS['small_board'] + self.get_image_width(self.images['gray_sq'])) + 2 * SPACINGS[
                    'board_to_info']
    ...
    def draw_screen(self):
        ...
        self.draw_text_rightof('x{0}'.format(self.state.stones[0]), 'monospace', self.text_height, color.WHITE,
                               'red_icon', 0, 0, False, None)
        self.draw_image_below('red_cap_icon' + s1, 'red_icon', 0, 5, True, 'red_cap_icon')
        self.draw_text_rightof('x{0}'.format(self.state.capstones[0]), 'monospace', self.text_height, color.WHITE,
                               'red_cap_icon', 0, 0, False, None)
        self.draw_image_below('blue_icon' + s2, 'red_cap_icon', 0, 5, True, 'blue_icon')
        self.draw_text_rightof('x{0}'.format(self.state.stones[1]), 'monospace', self.text_height, color.WHITE,
                               'blue_icon', 0, 0, False, None)
        self.draw_image_below('blue_cap_icon' + s2, 'blue_icon', 0, 5, True, 'blue_cap_icon')
        self.draw_text_rightof('x{0}'.format(self.state.capstones[1]), 'monospace', self.text_height, color.WHITE,
                               'blue_cap_icon', 0, 0, False, None)
        # draw buttons
        ...
        for r in range(self.state.size):
            for c in range(self.state.size):
                self.draw_cell(r, c)
        # draw territory info
        self.draw_text_below('territory:', 'monospace', self.text_height, color.WHITE, 'mini_board', 0, 0, True,
                             'territory')
        ...
        self.draw_text_below(self.get_time_left_str(0), 'monospace', self.text_height, color.RED, 'time', 0, 0, True,
                             'tred')
        self.draw_text_rightof(' vs ', 'monospace', self.text_height, color.WHITE, 'tred', 0, 0, True, 'vs2')
        self.draw_text_rightof(self.get_time_left_str(1), 'monospace', self.text_height, color.BLUE, 'vs2', 0, 0, True,
                               'tblue')
        # draw winner
        if self.state.game_over():
            self.draw_text_below('game over!', 'monospace', self.text_height, color.YELLOW, 'tred', 0, 0, True,
                                 'gameover')
            self.draw_text_below('winner: ', 'monospace', self.text_height, color.YELLOW, 'gameover', 0, 0, True,
                                 'winner')
        ...
        # draw info
        self.draw_text_below('current player: ', 'monospace', self.text_height, color.WHITE, 'board', 20, 0, True,
                             'curplayer')
        if self.state.cur_player == 0:
        ...
        if self.player_type[self.state.cur_player] == 'human':
            self.draw_text_below('human player', 'monospace', self.text_height, color.WHITE, 'curplayer', 0, 0, True,
                                 'type')
        else:
            self.draw_text_below('ai agent (' + self.agents[self.state.cur_player].get_name()[0:36] + ')', 'monospace',
                                 self.text_height, color.WHITE, 'curplayer', 0, 0, True, 'type')
        if self.state.invalid_player != None:
            self.draw_text_below('invalid action', 'monospace', self.text_height, color.WHITE, 'type', 0, 0, True,
                                 'invalid')
        else:
            ...
            for piece_type, owner in pieces:
                ...
                if i == 0:
                    ...
                    if piece_type == STANDING_STONE:
                        ...
                        dx = (w1 - w2) // 2
                    self.draw_image_above(image_id, 'cell_{0}_{1}_{2}'.format(r, c, i - 1), dx, 0, False,
                                          'cell_{0}_{1}_{2}'.format(r, c, i))
                i += 1
                left -= 1
        else:
            # draw normal
...
if __name__ == "__main__":
    ...
    if args.init != None:
        init_state = read_state_from_file(args.init)
